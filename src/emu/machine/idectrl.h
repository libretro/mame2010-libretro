...
struct _ide_config
{
	...
	uint32_t bmspace;			/* address space of bus master transfer */
};
...
uint32_t ide_controller_r(running_device *config, int reg, int size);
void ide_controller_w(running_device *config, int reg, int size, uint32_t data);
