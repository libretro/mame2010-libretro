...
INLINE uint8_t RB(sh4_state *sh4, offs_t A)
{
	if (A >= 0xfe000000)
...
INLINE uint16_t RW(sh4_state *sh4, offs_t A)
{
	if (A >= 0xfe000000)
...
INLINE uint32_t RL(sh4_state *sh4, offs_t A)
{
	if (A >= 0xfe000000)
...
INLINE void WB(sh4_state *sh4, offs_t A, uint8_t V)
{
...
INLINE void WW(sh4_state *sh4, offs_t A, uint16_t V)
{
	if (A >= 0xfe000000)
	{
...
INLINE void WL(sh4_state *sh4, offs_t A, uint32_t V)
{
	if (A >= 0xfe000000)
	{
 ...
 *  ADD     Rm,Rn
 */
INLINE void ADD(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] += sh4->r[m];
}
 ...
 *  ADD     #imm,Rn
 */
INLINE void ADDI(sh4_state *sh4, uint32_t i, uint32_t n)
{
	sh4->r[n] += (int32_t)(int16_t)(int8_t)i;
}
 ...
 *  ADDC    Rm,Rn
 */
INLINE void ADDC(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t tmp0, tmp1;
 ...
 *  ADDV    Rm,Rn
 */
INLINE void ADDV(sh4_state *sh4, uint32_t m, uint32_t n)
{
	int32_t dest, src, ans;

	if ((int32_t) sh4->r[n] >= 0)
		dest = 0;
	else
		dest = 1;
	if ((int32_t) sh4->r[m] >= 0)
		src = 0;
	else
		src = 1;
	src += dest;
 ...
 *  AND     Rm,Rn
 */
INLINE void AND(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] &= sh4->r[m];
}
 ...
 *  AND     #imm,R0
 */
INLINE void ANDI(sh4_state *sh4, uint32_t i)
{
	sh4->r[0] &= i;
}
 ...
 *  AND.B   #imm,@(R0,GBR)
 */
INLINE void ANDM(sh4_state *sh4, uint32_t i)
{
	uint32_t temp;
 ...
 *  BF      disp8
 */
INLINE void BF(sh4_state *sh4, uint32_t d)
{
	if ((sh4->sr & T) == 0)
	{
 ...
 *  BFS     disp8
 */
INLINE void BFS(sh4_state *sh4, uint32_t d)
{
	if ((sh4->sr & T) == 0)
	{
		int32_t disp = ((int32_t)d << 24) >> 24;
 ...
 *  BRA     disp12
 */
INLINE void BRA(sh4_state *sh4, uint32_t d)
{
	...
	if (disp == -2)
	{
		uint32_t next_opcode = RW(sh4,sh4->ppc & AM);
		/* BRA  $
         * NOP
         */
 ...
 *  BRAF    Rm
 */
INLINE void BRAF(sh4_state *sh4, uint32_t m)
{
	sh4->delay = sh4->pc;
	sh4->pc += sh4->r[m] + 2;
	sh4->sh4_icount--;
 ...
 *  BSR     disp12
 */
INLINE void BSR(sh4_state *sh4, uint32_t d)
{
	int32_t disp = ((int32_t)d << 20) >> 20;
 ...
 *  BSRF    Rm
 */
INLINE void BSRF(sh4_state *sh4, uint32_t m)
{
	sh4->pr = sh4->pc + 2;
	sh4->delay = sh4->pc;
	sh4->pc += sh4->r[m] + 2;
 ...
 *  BT      disp8
 */
INLINE void BT(sh4_state *sh4, uint32_t d)
{
	if ((sh4->sr & T) != 0)
	{
		int32_t disp = ((int32_t)d << 24) >> 24;
 ...
 *  BTS     disp8
 */
INLINE void BTS(sh4_state *sh4, uint32_t d)
{
	if ((sh4->sr & T) != 0)
	{
		int32_t disp = ((int32_t)d << 24) >> 24;
 ...
 *  CMP_EQ  Rm,Rn
 */
INLINE void CMPEQ(sh4_state *sh4, uint32_t m, uint32_t n)
{
	if (sh4->r[n] == sh4->r[m])
		sh4->sr |= T;
	else
 ...
 *  CMP_GE  Rm,Rn
 */
INLINE void CMPGE(sh4_state *sh4, uint32_t m, uint32_t n)
{
	if ((int32_t) sh4->r[n] >= (int32_t) sh4->r[m])
		sh4->sr |= T;
	else
		sh4->sr &= ~T;
}
 ...
 *  CMP_GT  Rm,Rn
 */
INLINE void CMPGT(sh4_state *sh4, uint32_t m, uint32_t n)
{
	if ((int32_t) sh4->r[n] > (int32_t) sh4->r[m])
		sh4->sr |= T;
	else
 ...
 *  CMP_HI  Rm,Rn
 */
INLINE void CMPHI(sh4_state *sh4, uint32_t m, uint32_t n)
{
	if ((uint32_t) sh4->r[n] > (uint32_t) sh4->r[m])
		sh4->sr |= T;
	else
		sh4->sr &= ~T;
}
 ...
 *  CMP_HS  Rm,Rn
 */
INLINE void CMPHS(sh4_state *sh4, uint32_t m, uint32_t n)
{
	if ((uint32_t) sh4->r[n] >= (uint32_t) sh4->r[m])
		sh4->sr |= T;
	else
 ...
 *  CMP_PL  Rn
 */
INLINE void CMPPL(sh4_state *sh4, uint32_t n)
{
	if ((int32_t) sh4->r[n] > 0)
		sh4->sr |= T;
	else
 ...
 *  CMP_PZ  Rn
 */
INLINE void CMPPZ(sh4_state *sh4, uint32_t n)
{
	if ((int32_t) sh4->r[n] >= 0)
		sh4->sr |= T;
	else
 ...
 * CMP_STR  Rm,Rn
 */
INLINE void CMPSTR(sh4_state *sh4, uint32_t m, uint32_t n)
 {
  uint32_t temp;
  int32_t HH, HL, LH, LL;
 ...
 *  CMP/EQ #imm,R0
 */
INLINE void CMPIM(sh4_state *sh4, uint32_t i)
{
 ...
 *  DIV0S   Rm,Rn
 */
INLINE void DIV0S(sh4_state *sh4, uint32_t m, uint32_t n)
{
 ...
 *  DIV1 Rm,Rn
 */
INLINE void DIV1(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t tmp0;
	uint32_t old_q;
...
/*  DMULS.L Rm,Rn */
INLINE void DMULS(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t RnL, RnH, RmL, RmH, Res0, Res1, Res2;
	uint32_t temp0, temp1, temp2, temp3;
	int32_t tempm, tempn, fnLmL;

	tempn = (int32_t) sh4->r[n];
	tempm = (int32_t) sh4->r[m];
	if (tempn < 0)
		tempn = 0 - tempn;
	if (tempm < 0)
		tempm = 0 - tempm;
	if ((int32_t) (sh4->r[n] ^ sh4->r[m]) < 0)
		fnLmL = -1;
	else
		fnLmL = 0;
...
/*  DMULU.L Rm,Rn */
INLINE void DMULU(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t RnL, RnH, RmL, RmH, Res0, Res1, Res2;
	uint32_t temp0, temp1, temp2, temp3;
...
/*  DT      Rn */
INLINE void DT(sh4_state *sh4, uint32_t n)
{
	sh4->r[n]--;
	if (sh4->r[n] == 0)
		sh4->sr |= T;
...
/*  EXTS.B  Rm,Rn */
INLINE void EXTSB(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = ((int32_t)sh4->r[m] << 24) >> 24;
}

/*  EXTS.W  Rm,Rn */
INLINE void EXTSW(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = ((int32_t)sh4->r[m] << 16) >> 16;
}

/*  EXTU.B  Rm,Rn */
INLINE void EXTUB(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = sh4->r[m] & 0x000000ff;
}

/*  EXTU.W  Rm,Rn */
INLINE void EXTUW(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = sh4->r[m] & 0x0000ffff;
}

/*  JMP     @Rm */
INLINE void JMP(sh4_state *sh4, uint32_t m)
{
	sh4->delay = sh4->pc;
...
/*  JSR     @Rm */
INLINE void JSR(sh4_state *sh4, uint32_t m)
{
	sh4->delay = sh4->pc;
	sh4->pr = sh4->pc + 2;
	sh4->pc = sh4->ea = sh4->r[m];
...
/*  LDC     Rm,SR */
INLINE void LDCSR(sh4_state *sh4, uint32_t m)
{
uint32_t reg;
...
/*  LDC     Rm,GBR */
INLINE void LDCGBR(sh4_state *sh4, uint32_t m)
{
	sh4->gbr = sh4->r[m];
}

/*  LDC     Rm,VBR */
INLINE void LDCVBR(sh4_state *sh4, uint32_t m)
{
	sh4->vbr = sh4->r[m];
}

/*  LDC.L   @Rm+,SR */
INLINE void LDCMSR(sh4_state *sh4, uint32_t m)
{
uint32_t old;
...
/*  LDC.L   @Rm+,GBR */
INLINE void LDCMGBR(sh4_state *sh4, uint32_t m)
{
	sh4->ea = sh4->r[m];
...
/*  LDC.L   @Rm+,VBR */
INLINE void LDCMVBR(sh4_state *sh4, uint32_t m)
{
	sh4->ea = sh4->r[m];
...
/*  LDS     Rm,MACH */
INLINE void LDSMACH(sh4_state *sh4, uint32_t m)
{
	sh4->mach = sh4->r[m];
}

/*  LDS     Rm,MACL */
INLINE void LDSMACL(sh4_state *sh4, uint32_t m)
{
	sh4->macl = sh4->r[m];
}

/*  LDS     Rm,PR */
INLINE void LDSPR(sh4_state *sh4, uint32_t m)
{
	sh4->pr = sh4->r[m];
}

/*  LDS.L   @Rm+,MACH */
INLINE void LDSMMACH(sh4_state *sh4, uint32_t m)
{
	sh4->ea = sh4->r[m];
...
/*  LDS.L   @Rm+,MACL */
INLINE void LDSMMACL(sh4_state *sh4, uint32_t m)
{
	sh4->ea = sh4->r[m];
...
/*  LDS.L   @Rm+,PR */
INLINE void LDSMPR(sh4_state *sh4, uint32_t m)
{
	sh4->ea = sh4->r[m];
...
/*  MAC.L   @Rm+,@Rn+ */
INLINE void MAC_L(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t RnL, RnH, RmL, RmH, Res0, Res1, Res2;
	uint32_t temp0, temp1, temp2, temp3;
	int32_t tempm, tempn, fnLmL;

	tempn = (int32_t) RL(sh4, sh4->r[n] );
	sh4->r[n] += 4;
	tempm = (int32_t) RL(sh4, sh4->r[m] );
	sh4->r[m] += 4;
...
/*  MAC.W   @Rm+,@Rn+ */
INLINE void MAC_W(sh4_state *sh4, uint32_t m, uint32_t n)
{
	int32_t tempm, tempn, dest, src, ans;
	uint32_t templ;

	tempn = (int32_t) RW(sh4, sh4->r[n] );
	sh4->r[n] += 2;
	tempm = (int32_t) RW(sh4, sh4->r[m] );
	sh4->r[m] += 2;
	templ = sh4->macl;
	tempm = ((int32_t) (short) tempn * (int32_t) (short) tempm);
	if ((int32_t) sh4->macl >= 0)
		dest = 0;
	else
		dest = 1;
	if ((int32_t) tempm >= 0)
...
/*  MOV     Rm,Rn */
INLINE void MOV(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = sh4->r[m];
}

/*  MOV.B   Rm,@Rn */
INLINE void MOVBS(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[n];
...
/*  MOV.W   Rm,@Rn */
INLINE void MOVWS(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[n];
...
/*  MOV.L   Rm,@Rn */
INLINE void MOVLS(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[n];
...
/*  MOV.B   @Rm,Rn */
INLINE void MOVBL(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[m];
	sh4->r[n] = (uint32_t)(int32_t)(int16_t)(int8_t) RB(sh4,  sh4->ea );
}

/*  MOV.W   @Rm,Rn */
INLINE void MOVWL(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[m];
	sh4->r[n] = (uint32_t)(int32_t)(int16_t) RW(sh4, sh4->ea );
}

/*  MOV.L   @Rm,Rn */
INLINE void MOVLL(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[m];
...
/*  MOV.B   Rm,@-Rn */
INLINE void MOVBM(sh4_state *sh4, uint32_t m, uint32_t n)
{
	/* SMG : bug fix, was reading sh4->r[n] */
	uint32_t data = sh4->r[m] & 0x000000ff;
...
/*  MOV.W   Rm,@-Rn */
INLINE void MOVWM(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t data = sh4->r[m] & 0x0000ffff;
...
/*  MOV.L   Rm,@-Rn */
INLINE void MOVLM(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t data = sh4->r[m];
...
/*  MOV.B   @Rm+,Rn */
INLINE void MOVBP(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = (uint32_t)(int32_t)(int16_t)(int8_t) RB(sh4,  sh4->r[m] );
	if (n != m)
		sh4->r[m] += 1;
}

/*  MOV.W   @Rm+,Rn */
INLINE void MOVWP(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = (uint32_t)(int32_t)(int16_t) RW(sh4, sh4->r[m] );
	if (n != m)
		sh4->r[m] += 2;
}

/*  MOV.L   @Rm+,Rn */
INLINE void MOVLP(sh4_state *sh4, uint32_t m, uint32_t n)
{
...
/*  MOV.B   Rm,@(R0,Rn) */
INLINE void MOVBS0(sh4_state *sh4, uint32_t m, uint32_t n)
{
...
/*  MOV.W   Rm,@(R0,Rn) */
INLINE void MOVWS0(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[n] + sh4->r[0];
	WW(sh4, sh4->ea, sh4->r[m] & 0x0000ffff );
}
...
/*  MOV.L   Rm,@(R0,Rn) */
INLINE void MOVLS0(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[n] + sh4->r[0];
	WL(sh4, sh4->ea, sh4->r[m] );
}
...
/*  MOV.B   @(R0,Rm),Rn */
INLINE void MOVBL0(sh4_state *sh4, uint32_t m, uint32_t n)
{
...
/*  MOV.W   @(R0,Rm),Rn */
INLINE void MOVWL0(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[m] + sh4->r[0];
	sh4->r[n] = (uint32_t)(int32_t)(int16_t) RW(sh4, sh4->ea );
}
...
/*  MOV.L   @(R0,Rm),Rn */
INLINE void MOVLL0(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[m] + sh4->r[0];
	sh4->r[n] = RL(sh4, sh4->ea );
}
...
/*  MOV     #imm,Rn */
INLINE void MOVI(sh4_state *sh4, uint32_t i, uint32_t n)
{
	sh4->r[n] = (uint32_t)(int32_t)(int16_t)(int8_t) i;
}

/*  MOV.W   @(disp8,PC),Rn */
INLINE void MOVWI(sh4_state *sh4, uint32_t d, uint32_t n)
{
...
/*  MOV.L   @(disp8,PC),Rn */
INLINE void MOVLI(sh4_state *sh4, uint32_t d, uint32_t n)
{
	uint32_t disp = d & 0xff;
...
/*  MOV.B   @(disp8,GBR),R0 */
INLINE void MOVBLG(sh4_state *sh4, uint32_t d)
{
	uint32_t disp = d & 0xff;
	sh4->ea = sh4->gbr + disp;
	sh4->r[0] = (uint32_t)(int32_t)(int16_t)(int8_t) RB(sh4,  sh4->ea );
}

/*  MOV.W   @(disp8,GBR),R0 */
INLINE void MOVWLG(sh4_state *sh4, uint32_t d)
{
	uint32_t disp = d & 0xff;
	sh4->ea = sh4->gbr + disp * 2;
	sh4->r[0] = (int32_t)(int16_t) RW(sh4, sh4->ea );
}

/*  MOV.L   @(disp8,GBR),R0 */
INLINE void MOVLLG(sh4_state *sh4, uint32_t d)
{
	uint32_t disp = d & 0xff;
...
/*  MOV.B   R0,@(disp8,GBR) */
INLINE void MOVBSG(sh4_state *sh4, uint32_t d)
{
	uint32_t disp = d & 0xff;
	sh4->ea = sh4->gbr + disp;
...
/*  MOV.W   R0,@(disp8,GBR) */
INLINE void MOVWSG(sh4_state *sh4, uint32_t d)
{
	uint32_t disp = d & 0xff;
	sh4->ea = sh4->gbr + disp * 2;
...
/*  MOV.L   R0,@(disp8,GBR) */
INLINE void MOVLSG(sh4_state *sh4, uint32_t d)
{
	uint32_t disp = d & 0xff;
	sh4->ea = sh4->gbr + disp * 4;
...
/*  MOV.B   R0,@(disp4,Rn) */
INLINE void MOVBS4(sh4_state *sh4, uint32_t d, uint32_t n)
{
...
/*  MOV.W   R0,@(disp4,Rn) */
INLINE void MOVWS4(sh4_state *sh4, uint32_t d, uint32_t n)
{
	uint32_t disp = d & 0x0f;
...
/* MOV.L Rm,@(disp4,Rn) */
INLINE void MOVLS4(sh4_state *sh4, uint32_t m, uint32_t d, uint32_t n)
{
	uint32_t disp = d & 0x0f;
...
/*  MOV.B   @(disp4,Rm),R0 */
INLINE void MOVBL4(sh4_state *sh4, uint32_t m, uint32_t d)
{
	uint32_t disp = d & 0x0f;
	sh4->ea = sh4->r[m] + disp;
	sh4->r[0] = (uint32_t)(int32_t)(int16_t)(int8_t) RB(sh4,  sh4->ea );
...
/*  MOV.W   @(disp4,Rm),R0 */
INLINE void MOVWL4(sh4_state *sh4, uint32_t m, uint32_t d)
{
	uint32_t disp = d & 0x0f;
	sh4->ea = sh4->r[m] + disp * 2;
	sh4->r[0] = (uint32_t)(int32_t)(int16_t) RW(sh4, sh4->ea );
}

/*  MOV.L   @(disp4,Rm),Rn */
INLINE void MOVLL4(sh4_state *sh4, uint32_t m, uint32_t d, uint32_t n)
{
	uint32_t disp = d & 0x0f;
...
/*  MOVA    @(disp8,PC),R0 */
INLINE void MOVA(sh4_state *sh4, uint32_t d)
{
	uint32_t disp = d & 0xff;
	sh4->ea = ((sh4->pc + 2) & ~3) + disp * 4;
	sh4->r[0] = sh4->ea;
...
/*  MOVT    Rn */
INLINE void MOVT(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->sr & T;
}

/*  MUL.L   Rm,Rn */
INLINE void MULL(sh4_state *sh4, uint32_t m, uint32_t n)
{
...
/*  MULS    Rm,Rn */
INLINE void MULS(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->macl = (int16_t) sh4->r[n] * (int16_t) sh4->r[m];
}

/*  MULU    Rm,Rn */
INLINE void MULU(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->macl = (uint16_t) sh4->r[n] * (uint16_t) sh4->r[m];
}

/*  NEG     Rm,Rn */
INLINE void NEG(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = 0 - sh4->r[m];
}

/*  NEGC    Rm,Rn */
INLINE void NEGC(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t temp;
...
/*  NOT     Rm,Rn */
INLINE void NOT(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = ~sh4->r[m];
}

/*  OR      Rm,Rn */
INLINE void OR(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] |= sh4->r[m];
}

/*  OR      #imm,R0 */
INLINE void ORI(sh4_state *sh4, uint32_t i)
{
	sh4->r[0] |= i;
	sh4->sh4_icount -= 2;
}
...
/*  OR.B    #imm,@(R0,GBR) */
INLINE void ORM(sh4_state *sh4, uint32_t i)
{
	uint32_t temp;
...
/*  ROTCL   Rn */
INLINE void ROTCL(sh4_state *sh4, uint32_t n)
{
	uint32_t temp;
...
/*  ROTCR   Rn */
INLINE void ROTCR(sh4_state *sh4, uint32_t n)
{
	uint32_t temp;
	temp = (sh4->sr & T) << 31;
	if (sh4->r[n] & T)
...
/*  ROTL    Rn */
INLINE void ROTL(sh4_state *sh4, uint32_t n)
{
...
/*  ROTR    Rn */
INLINE void ROTR(sh4_state *sh4, uint32_t n)
{
...
INLINE void SHAL(sh4_state *sh4, uint32_t n)
{
	sh4->sr = (sh4->sr & ~T) | ((sh4->r[n] >> 31) & T);
	sh4->r[n] <<= 1;
}
...
/*  SHAR    Rn */
INLINE void SHAR(sh4_state *sh4, uint32_t n)
{
	sh4->sr = (sh4->sr & ~T) | (sh4->r[n] & T);
	sh4->r[n] = (uint32_t)((int32_t)sh4->r[n] >> 1);
}

/*  SHLL    Rn      (same as SHAL) */
INLINE void SHLL(sh4_state *sh4, uint32_t n)
{
	sh4->sr = (sh4->sr & ~T) | ((sh4->r[n] >> 31) & T);
	sh4->r[n] <<= 1;
}
...
/*  SHLL2   Rn */
INLINE void SHLL2(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] <<= 2;
}

/*  SHLL8   Rn */
INLINE void SHLL8(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] <<= 8;
}

/*  SHLL16  Rn */
INLINE void SHLL16(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] <<= 16;
}

/*  SHLR    Rn */
INLINE void SHLR(sh4_state *sh4, uint32_t n)
{
	sh4->sr = (sh4->sr & ~T) | (sh4->r[n] & T);
	sh4->r[n] >>= 1;
}
...
/*  SHLR2   Rn */
INLINE void SHLR2(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] >>= 2;
}

/*  SHLR8   Rn */
INLINE void SHLR8(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] >>= 8;
}

/*  SHLR16  Rn */
INLINE void SHLR16(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] >>= 16;
}
...
/*  STC     SR,Rn */
INLINE void STCSR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->sr;
}

/*  STC     GBR,Rn */
INLINE void STCGBR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->gbr;
}

/*  STC     VBR,Rn */
INLINE void STCVBR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->vbr;
}

/*  STC.L   SR,@-Rn */
INLINE void STCMSR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STC.L   GBR,@-Rn */
INLINE void STCMGBR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STC.L   VBR,@-Rn */
INLINE void STCMVBR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STS     MACH,Rn */
INLINE void STSMACH(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->mach;
}

/*  STS     MACL,Rn */
INLINE void STSMACL(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->macl;
}

/*  STS     PR,Rn */
INLINE void STSPR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->pr;
}

/*  STS.L   MACH,@-Rn */
INLINE void STSMMACH(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STS.L   MACL,@-Rn */
INLINE void STSMMACL(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STS.L   PR,@-Rn */
INLINE void STSMPR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  SUB     Rm,Rn */
INLINE void SUB(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] -= sh4->r[m];
}

/*  SUBC    Rm,Rn */
INLINE void SUBC(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t tmp0, tmp1;
...
/*  SUBV    Rm,Rn */
INLINE void SUBV(sh4_state *sh4, uint32_t m, uint32_t n)
{
	int32_t dest, src, ans;
...
/*  SWAP.B  Rm,Rn */
INLINE void SWAPB(sh4_state *sh4, uint32_t m, uint32_t n)
{
...
/*  SWAP.W  Rm,Rn */
INLINE void SWAPW(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t temp;
...
/*  TAS.B   @Rn */
INLINE void TAS(sh4_state *sh4, uint32_t n)
{
	uint32_t temp;
	sh4->ea = sh4->r[n];
	/* Bus Lock enable */
...
/*  TRAPA   #imm */
INLINE void TRAPA(sh4_state *sh4, uint32_t i)
{
	uint32_t imm = i & 0xff;
...
/*  TST     Rm,Rn */
INLINE void TST(sh4_state *sh4, uint32_t m, uint32_t n)
{
...
/*  TST     #imm,R0 */
INLINE void TSTI(sh4_state *sh4, uint32_t i)
{
	uint32_t imm = i & 0xff;
...
/*  TST.B   #imm,@(R0,GBR) */
INLINE void TSTM(sh4_state *sh4, uint32_t i)
{
	uint32_t imm = i & 0xff;
...
/*  XOR     Rm,Rn */
INLINE void XOR(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] ^= sh4->r[m];
}

/*  XOR     #imm,R0 */
INLINE void XORI(sh4_state *sh4, uint32_t i)
{
	uint32_t imm = i & 0xff;
	sh4->r[0] ^= imm;
}
...
/*  XOR.B   #imm,@(R0,GBR) */
INLINE void XORM(sh4_state *sh4, uint32_t i)
{
	uint32_t imm = i & 0xff;
	uint32_t temp;
...
/*  XTRCT   Rm,Rn */
INLINE void XTRCT(sh4_state *sh4, uint32_t m, uint32_t n)
{
	uint32_t temp;
...
/*  STC     SSR,Rn */
INLINE void STCSSR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->ssr;
}

/*  STC     SPC,Rn */
INLINE void STCSPC(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->spc;
}

/*  STC     SGR,Rn */
INLINE void STCSGR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->sgr;
}

/*  STS     FPUL,Rn */
INLINE void STSFPUL(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->fpul;
}

/*  STS     FPSCR,Rn */
INLINE void STSFPSCR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->fpscr & 0x003FFFFF;
}

/*  STC     DBR,Rn */
INLINE void STCDBR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] = sh4->dbr;
}

/*  STCRBANK   Rm_BANK,Rn */
INLINE void STCRBANK(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] = sh4->rbnk[sh4->sr&sRB ? 0 : 1][m & 7];
}

/*  STCMRBANK   Rm_BANK,@-Rn */
INLINE void STCMRBANK(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  MOVCA.L     R0,@Rn */
INLINE void MOVCAL(sh4_state *sh4, uint32_t n)
{
	sh4->ea = sh4->r[n];
	WL(sh4, sh4->ea, sh4->r[0] );
}
...
/*  STS.L   SGR,@-Rn */
INLINE void STCMSGR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STS.L   FPUL,@-Rn */
INLINE void STSMFPUL(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STS.L   FPSCR,@-Rn */
INLINE void STSMFPSCR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STC.L   DBR,@-Rn */
INLINE void STCMDBR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STC.L   SSR,@-Rn */
INLINE void STCMSSR(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  STC.L   SPC,@-Rn */
INLINE void STCMSPC(sh4_state *sh4, uint32_t n)
{
	sh4->r[n] -= 4;
	sh4->ea = sh4->r[n];
...
/*  LDS.L   @Rm+,FPUL */
INLINE void LDSMFPUL(sh4_state *sh4, uint32_t m)
{
	sh4->ea = sh4->r[m];
...
/*  LDS.L   @Rm+,FPSCR */
INLINE void LDSMFPSCR(sh4_state *sh4, uint32_t m)
{
uint32_t s;
...
/*  LDC.L   @Rm+,DBR */
INLINE void LDCMDBR(sh4_state *sh4, uint32_t m)
{
	sh4->ea = sh4->r[m];
...
/*  LDC.L   @Rn+,Rm_BANK */
INLINE void LDCMRBANK(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->ea = sh4->r[n];
...
/*  LDC.L   @Rm+,SSR */
INLINE void LDCMSSR(sh4_state *sh4, uint32_t m)
{
	sh4->ea = sh4->r[m];
...
/*  LDC.L   @Rm+,SPC */
INLINE void LDCMSPC(sh4_state *sh4, uint32_t m)
{
	sh4->ea = sh4->r[m];
...
/*  LDS     Rm,FPUL */
INLINE void LDSFPUL(sh4_state *sh4, uint32_t m)
{
	sh4->fpul = sh4->r[m];
}

/*  LDS     Rm,FPSCR */
INLINE void LDSFPSCR(sh4_state *sh4, uint32_t m)
{
uint32_t s;
...
/*  LDC     Rm,DBR */
INLINE void LDCDBR(sh4_state *sh4, uint32_t m)
{
	sh4->dbr = sh4->r[m];
}

/*  SHAD    Rm,Rn */
INLINE void SHAD(sh4_state *sh4, uint32_t m,uint32_t n)
{
	if ((sh4->r[m] & 0x80000000) == 0)
		...
		sh4->r[n]=(int32_t)sh4->r[n] >> ((~sh4->r[m] & 0x1F)+1);
}

/*  SHLD    Rm,Rn */
INLINE void SHLD(sh4_state *sh4, uint32_t m,uint32_t n)
{
	if ((sh4->r[m] & 0x80000000) == 0)
		sh4->r[n] = sh4->r[n] << (sh4->r[m] & 0x1F);
	else if ((sh4->r[m] & 0x1F) == 0)
...
/*  LDCRBANK   Rn,Rm_BANK */
INLINE void LDCRBANK(sh4_state *sh4, uint32_t m, uint32_t n)
{
	sh4->rbnk[sh4->sr&sRB ? 0 : 1][m & 7] = sh4->r[n];
}

/*  LDC     Rm,SSR */
INLINE void LDCSSR(sh4_state *sh4, uint32_t m)
{
	sh4->ssr = sh4->r[m];
}

/*  LDC     Rm,SPC */
INLINE void LDCSPC(sh4_state *sh4, uint32_t m)
{
	sh4->spc = sh4->r[m];
}

static uint32_t sh4_getsqremap(sh4_state *sh4, uint32_t address)
{
	...
	else
	{
		...
		for (i=0;i<64;i++)
		{
			uint32_t topcmp = sh4->sh4_tlb_address[i]&0xfff00000;
			if (topcmp==topaddr)
...
/*  PREF     @Rn */
INLINE void PREFM(sh4_state *sh4, uint32_t n)
{
	int a;
 ...
 *  OPCODE DISPATCHERS
 *****************************************************************************/
INLINE void op0000(sh4_state *sh4, uint16_t opcode)
{
	switch (opcode & 0xF)
	{
	case 0x0:
...
INLINE void op0001(sh4_state *sh4, uint16_t opcode)
{
	MOVLS4(sh4, Rm, opcode & 0x0f, Rn);
}

INLINE void op0010(sh4_state *sh4, uint16_t opcode)
{
	switch (opcode & 15)
	{
...
INLINE void op0011(sh4_state *sh4, uint16_t opcode)
{
	switch (opcode & 15)
	{
...
INLINE void op0100(sh4_state *sh4, uint16_t opcode)
{
	switch (opcode & 0xF)
	{
	case 0x0:
...
INLINE void op0101(sh4_state *sh4, uint16_t opcode)
{
	MOVLL4(sh4, Rm, opcode & 0x0f, Rn);
}

INLINE void op0110(sh4_state *sh4, uint16_t opcode)
{
	switch (opcode & 15)
	{
...
INLINE void op0111(sh4_state *sh4, uint16_t opcode)
{
	ADDI(sh4, opcode & 0xff, Rn);
}

INLINE void op1000(sh4_state *sh4, uint16_t opcode)
{
	switch ( opcode  & (15<<8) )
	{
...
INLINE void op1001(sh4_state *sh4, uint16_t opcode)
{
	MOVWI(sh4, opcode & 0xff, Rn);
}

INLINE void op1010(sh4_state *sh4, uint16_t opcode)
{
	BRA(sh4, opcode & 0xfff);
}

INLINE void op1011(sh4_state *sh4, uint16_t opcode)
{
	BSR(sh4, opcode & 0xfff);
}

INLINE void op1100(sh4_state *sh4, uint16_t opcode)
{
	switch (opcode & (15<<8))
	{
...
INLINE void op1101(sh4_state *sh4, uint16_t opcode)
{
	MOVLI(sh4, opcode & 0xff, Rn);
}

INLINE void op1110(sh4_state *sh4, uint16_t opcode)
{
...
INLINE void FMOVMRIFR(sh4_state *sh4, uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		sh4->ea = sh4->r[m];
...
INLINE void FMOVFRMR(sh4_state *sh4,uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		m= m & 14;
		sh4->ea = sh4->r[n];
...
INLINE void FMOVFRMDR(sh4_state *sh4,uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		m= m & 14;
		sh4->r[n] -= 8;
...
INLINE void FMOVFRS0(sh4_state *sh4,uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		m= m & 14;
...
INLINE void FMOVS0FR(sh4_state *sh4,uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n= n & 14;
...
INLINE void FMOVMRFR(sh4_state *sh4,uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		if (n & 1) {
			n= n & 14;
...
INLINE void FMOVFR(sh4_state *sh4,uint32_t m,uint32_t n)
{
...
INLINE void FLDI1(sh4_state *sh4, uint32_t n)
{
	sh4->fr[n] = 0x3F800000;
}

/*  FLDI0  FRn 1111nnnn10001101 */
INLINE void FLDI0(sh4_state *sh4, uint32_t n)
{
	sh4->fr[n] = 0;
}

/*  FLDS FRm,FPUL 1111mmmm00011101 */
INLINE void	FLDS(sh4_state *sh4, uint32_t m)
{
	sh4->fpul = sh4->fr[m];
}

/*  FSTS FPUL,FRn 1111nnnn00001101 */
INLINE void	FSTS(sh4_state *sh4, uint32_t n)
{
	sh4->fr[n] = sh4->fpul;
}
...
INLINE void FTRC(sh4_state *sh4, uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		*((int32_t *)&sh4->fpul) = (int32_t)FP_RFD(n);
	} else {              /* PR = 0 */
		/* read sh4->fr[n] as float -> truncate -> fpul(32) */
		*((int32_t *)&sh4->fpul) = (int32_t)FP_RFS(n);
	}
}

/* FLOAT FPUL,FRn PR=0 1111nnnn00101101 */
/* FLOAT FPUL,DRn PR=1 1111nnn000101101 */
INLINE void FLOAT(sh4_state *sh4, uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		FP_RFD(n) = (double)*((int32_t *)&sh4->fpul);
	} else {              /* PR = 0 */
		FP_RFS(n) = (float)*((int32_t *)&sh4->fpul);
	}
}

/* FNEG FRn PR=0 1111nnnn01001101 */
/* FNEG DRn PR=1 1111nnn001001101 */
INLINE void FNEG(sh4_state *sh4, uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		FP_RFD(n) = -FP_RFD(n);
	} else {              /* PR = 0 */
...
INLINE void FABS(sh4_state *sh4, uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
#ifdef MSB_FIRST
		n = n & 14;
...
INLINE void FCMP_EQ(sh4_state *sh4, uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		m = m & 14;
...
INLINE void FCMP_GT(sh4_state *sh4, uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		m = m & 14;
...
INLINE void FCNVDS(sh4_state *sh4, uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		if (sh4->fpscr & RM)
...
INLINE void FCNVSD(sh4_state *sh4, uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
...
INLINE void FADD(sh4_state *sh4, uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		m = m & 14;
...
INLINE void FSUB(sh4_state *sh4, uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		m = m & 14;
...
INLINE void FMUL(sh4_state *sh4, uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		m = m & 14;
...
INLINE void FDIV(sh4_state *sh4, uint32_t m,uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		m = m & 14;
...
INLINE void FMAC(sh4_state *sh4, uint32_t m,uint32_t n)
{
...
INLINE void FSQRT(sh4_state *sh4, uint32_t n)
{
	if (sh4->fpu_pr) { /* PR = 1 */
		n = n & 14;
		if (FP_RFD(n) < 0)
...
INLINE void FSRRA(sh4_state *sh4, uint32_t n)
{
	if (FP_RFS(n) < 0)
		return;
...
INLINE void FSSCA(sh4_state *sh4, uint32_t n)
{
float angle;
...
INLINE void FIPR(sh4_state *sh4, uint32_t n)
{
uint32_t m;
float ml[4];
int a;
...
INLINE void FTRV(sh4_state *sh4, uint32_t n)
{
int i,j;
float sum[4];
...
INLINE void op1111(sh4_state *sh4, uint16_t opcode)
{
	switch (opcode & 0xf)
	{
		case 0:
...
static CPU_EXECUTE( sh4 )
{
	...
	do
	{
		...
		if (sh4->delay)
		{
			opcode = memory_decrypted_read_word(sh4->program, WORD2_XOR_LE((uint32_t)(sh4->delay & AM)));
			sh4->pc -= 2;
		}
		else
			opcode = memory_decrypted_read_word(sh4->program, WORD2_XOR_LE((uint32_t)(sh4->pc & AM)));
