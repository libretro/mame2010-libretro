...
INLINE char *signed_16bit_unary(int16_t val)
{
	static char temp[10];
	if (val < 0)
		sprintf(temp, "-$%x", -val);
...
INLINE char *signed_16bit_sep(int16_t val)
{
	static char temp[10];
	if (val < 0)
		sprintf(temp, " - $%x", -val);
...
INLINE char *signed_16bit_sep_nospace(int16_t val)
{
	static char temp[10];
	if (val < 0)
		sprintf(temp, "-$%x", -val);
...
INLINE char *unsigned_16bit_size(int16_t val, uint8_t size)
{
	static char temp[10];
	if (size)
