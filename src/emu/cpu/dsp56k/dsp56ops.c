...
static size_t dsp56k_op_movec(dsp56k_core* cpustate, const uint16_t op, uint8_t* cycles)
{
	...
	{
		/* Write D */
		uint16_t value = memory_read_word_16le(cpustate->data, ADDRESS(*((uint16_t*)R.addr))) ;
...
static size_t dsp56k_op_movec_1(dsp56k_core* cpustate, const uint16_t op, uint8_t* cycles)
{
	...
	{
		/* Write D */
		uint16_t tempData = memory_read_word_16le(cpustate->data, ADDRESS(memOffset));
...
static size_t dsp56k_op_movec_3(dsp56k_core* cpustate, const uint16_t op, const uint16_t op2, uint8_t* cycles)
{
	...
	if (W)
	{
		...
		{
			/* 16-bit long address */
			uint16_t tempD = memory_read_word_16le(cpustate->data, ADDRESS(op2));
...
static size_t dsp56k_op_movem(dsp56k_core* cpustate, const uint16_t op, uint8_t* cycles)
{
	...
	if (W)
	{
		...
		typed_pointer data;
		uint16_t ldata = memory_read_word_16le(cpustate->program, ADDRESS(*((uint16_t*)R.addr)));
...
static size_t dsp56k_op_movep(dsp56k_core* cpustate, const uint16_t op, uint8_t* cycles)
{
	...
	if (W)
	{
		uint16_t data = memory_read_word_16le(cpustate->data, ADDRESS(pp));
...
static size_t dsp56k_op_movep_1(dsp56k_core* cpustate, const uint16_t op, uint8_t* cycles)
{
	...
	if (W)
	{
		uint16_t data = memory_read_word_16le(cpustate->data, ADDRESS(*((uint16_t*)SD.addr)));
...
static void execute_x_memory_data_move(dsp56k_core* cpustate, const uint16_t op, typed_pointer* d_register, uint64_t* prev_accum_value)
{
	...
	{
		/* From X:<ea> to SD */
		uint16_t data = memory_read_word_16le(cpustate->data, ADDRESS(*((uint16_t*)R.addr)));
...
static void execute_x_memory_data_move2(dsp56k_core* cpustate, const uint16_t op, typed_pointer* d_register)
{
	...
	{
		/* Write D */
		uint16_t value = memory_read_word_16le(cpustate->data, ADDRESS(*mem_offset));
...
static void execute_dual_x_memory_data_read(dsp56k_core* cpustate, const uint16_t op, typed_pointer* d_register)
{
	...
	/* First memmove */
	srcVal1 = memory_read_word_16le(cpustate->data, ADDRESS(*((uint16_t*)R.addr)));
	tempV.addr = &srcVal1;
...
static void SetDataMemoryValue(dsp56k_core* cpustate, typed_pointer source, uint32_t destinationAddr)
{
	switch(source.data_type)
	{
		case DT_BYTE:        memory_write_word_16le(cpustate->data, destinationAddr, (uint16_t)( (*((uint8_t*) source.addr) & 0xff)               ) ) ; break ;
		case DT_WORD:        memory_write_word_16le(cpustate->data, destinationAddr, (uint16_t)( (*((uint16_t*)source.addr) & 0xffff)             ) ) ; break ;
		case DT_DOUBLE_WORD: memory_write_word_16le(cpustate->data, destinationAddr, (uint16_t)( (*((uint32_t*)source.addr) & 0x0000ffff)         ) ) ; break ;

		/* !!! Is this universal ??? */
		/* !!! Forget not, yon shift-limiter !!! */
		case DT_LONG_WORD:   memory_write_word_16le(cpustate->data, destinationAddr, (uint16_t)( ((*((uint64_t*)source.addr)) & U64(0x00000000ffff0000)) >> 16) ) ; break ;
	}
}
...
static void SetProgramMemoryValue(dsp56k_core* cpustate, typed_pointer source, uint32_t destinationAddr)
{
	switch(source.data_type)
	{
		case DT_BYTE:        memory_write_word_16le(cpustate->program, destinationAddr, (uint16_t)( (*((uint8_t*) source.addr) & 0xff)               ) ) ; break ;
		case DT_WORD:        memory_write_word_16le(cpustate->program, destinationAddr, (uint16_t)( (*((uint16_t*)source.addr) & 0xffff)             ) ) ; break ;
		case DT_DOUBLE_WORD: memory_write_word_16le(cpustate->program, destinationAddr, (uint16_t)( (*((uint32_t*)source.addr) & 0x0000ffff)         ) ) ; break ;

		/* !!! Is this universal ??? */
		/* !!! Forget not, yon shift-limiter !!! */
		case DT_LONG_WORD:   memory_write_word_16le(cpustate->program, destinationAddr, (uint16_t)( ((*((uint64_t*)source.addr)) & U64(0x00000000ffff0000)) >> 16) ) ; break ;
	}
}
