...
static char *effective_address( DasmPSXCPU_state *state, uint32_t pc, uint32_t op )
{
...
static uint32_t relative_address( DasmPSXCPU_state *state, uint32_t pc, uint32_t op )
{
...
static uint32_t jump_address( DasmPSXCPU_state *state, uint32_t pc, uint32_t op )
{
	uint32_t nextpc = pc + 4;
	if( state != NULL && state->pc == pc && state->delayr == PSXCPU_DELAYR_PC )
	{
...
unsigned DasmPSXCPU( DasmPSXCPU_state *state, char *buffer, uint32_t pc, const uint8_t *opram )
{
	uint32_t op;
