...
struct _work_unit_shared
{
	...
	volatile uint32_t		count_next;				/* number of scanlines and index of next item to process */
...
INLINE void convert_tri_extent_to_poly_extent(poly_extent *dstextent, const tri_extent *srcextent, const polygon_info *polygon, int32_t y)
{
	...
	while (1)
	{
		...
		if (unit->shared.previtem != 0xffff)
		{
			...
			if (prevunit->shared.count_next != 0)
			{
				...
				} while (compare_exchange32((volatile int32_t *)&prevunit->shared.count_next, orig_count_next, new_count_next) != orig_count_next);
		...
		} while (compare_exchange32((volatile int32_t *)&unit->shared.count_next, orig_count_next, 0) != orig_count_next);
