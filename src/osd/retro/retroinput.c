...
/*
static int getButtonValue(int button, int layoutIndex,int joy) {

	if (button == 4) {
		return RETRO_DEVICE_ID_JOYPAD_Y+joy;
	}

	if (layoutIndex == 0) {
		switch (button) {
		case 1 : return RETRO_DEVICE_ID_JOYPAD_A+joy;
		case 2 : return RETRO_DEVICE_ID_JOYPAD_B+joy;
		case 3 : return RETRO_DEVICE_ID_JOYPAD_X+joy;
		}
	} else
	if (layoutIndex == 1) {
		switch (button) {
		case 1 : return RETRO_DEVICE_ID_JOYPAD_B+joy;
		case 2 : return RETRO_DEVICE_ID_JOYPAD_A+joy;
		case 3 : return RETRO_DEVICE_ID_JOYPAD_X+joy;
		}
	} else
	if (layoutIndex == 2) {
		switch (button) {
		case 1 : return RETRO_DEVICE_ID_JOYPAD_X+joy;
		case 2 : return RETRO_DEVICE_ID_JOYPAD_A+joy;
		case 3 : return RETRO_DEVICE_ID_JOYPAD_B+joy;
		}
	}
	return 0;
}
...
static void initInput(running_machine* machine) {
	...
	P1_state[KEY_BUTTON_1] = 0;//getButtonValue(1, optButtonLayoutP1,0);
	input_device_item_add(P1_device, "P1 B1", &P1_state[KEY_BUTTON_1], ITEM_ID_LCONTROL, pad1_get_state);
	P1_state[KEY_BUTTON_2] = 0;//getButtonValue(2, optButtonLayoutP1,0);
	input_device_item_add(P1_device, "P1 B2", &P1_state[KEY_BUTTON_2], ITEM_ID_LALT, pad1_get_state);
	P1_state[KEY_BUTTON_3] = 0;//getButtonValue(3, optButtonLayoutP1,0);
