...
static uint16_t *bmcbowl_vid1;
static uint16_t *bmcbowl_vid2;
static uint8_t *bmc_colorram;

static uint8_t *stats_ram;
...
static void init_stats(const uint8_t *table, int table_len, int address)
{
	int i;
...
static ADDRESS_MAP_START( bmcbowl_mem, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x095000, 0x095fff) AM_RAM AM_BASE((uint16_t **)&stats_ram) AM_SIZE(&stats_ram_size) /* 8 bit */
