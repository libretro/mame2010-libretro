...
static TILE_GET_INFO( get_fg_tile_info )
{
	...
	uint8_t attr = state->xevious_fg_colorram[tile_index];
...
static TILE_GET_INFO( get_bg_tile_info )
{
	...
	uint8_t code = state->xevious_bg_videoram[tile_index];
	uint8_t attr = state->xevious_bg_colorram[tile_index];
...
READ8_HANDLER( xevious_bb_r )
{
	...
	uint8_t *rom2a = memory_region(space->machine, "gfx4");
...
static void draw_sprites(running_machine *machine, bitmap_t *bitmap,const rectangle *cliprect)
{
	...
	uint8_t *spriteram = state->xevious_sr3 + 0x780;
	uint8_t *spriteram_2 = state->xevious_sr1 + 0x780;
	uint8_t *spriteram_3 = state->xevious_sr2 + 0x780;
	int offs,sx,sy;
