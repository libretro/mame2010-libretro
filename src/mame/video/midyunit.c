...
{
	/* allocate memory */
	midyunit_cmos_ram = auto_alloc_array(machine, uint16_t, (0x2000 * 4)/2);
	local_videoram = auto_alloc_array_clear(machine, uint16_t, 0x80000/2);
...
static void dma_draw(uint16_t command)
{
...
if (LOG_DMA)
{
	if (input_code_pressed(space->machine, KEYCODE_L))
	{
		...
		logerror("  offset=%08X pos=(%d,%d) w=%d h=%d rb=%d\n",
				...
				dma_register[DMA_WIDTH], dma_register[DMA_HEIGHT], (int16_t)dma_register[DMA_ROWBYTES]);
...
static TIMER_CALLBACK( autoerase_line )
{
	...
	if (autoerase_enable && scanline >= 0 && scanline < 510)
		memcpy(&local_videoram[512 * scanline], &local_videoram[512 * (510 + (scanline & 1))], 512 * sizeof(uint16_t));
}
