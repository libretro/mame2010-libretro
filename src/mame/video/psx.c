...
static uint32_t m_n_lightgun_x;
static uint32_t m_n_lightgun_y;
...
static uint16_t m_p_n_g0r0[ 0x10000 ];
static uint16_t m_p_n_b0[ 0x10000 ];
static uint16_t m_p_n_r1[ 0x10000 ];
static uint16_t m_p_n_b1g1[ 0x10000 ];
...
static void psx_gpu_init( running_machine *machine )
{
	...
	m_p_vram = auto_alloc_array_clear(machine, uint16_t, m_n_vram_size );
	...
	// icky!!!
	state_save_register_memory( machine, "globals", NULL, 0, "m_packet", (uint8_t *)&m_packet, 1, sizeof( m_packet ), __FILE__, __LINE__ );
...
VIDEO_UPDATE( psx )
{
	...
	else
	{
		...
		n_top = (int32_t)m_n_vert_disstart - n_overscantop;
		n_lines = (int32_t)m_n_vert_disend - (int32_t)m_n_vert_disstart;
		if( n_top < 0 )
		{
			n_y = -n_top;
			n_lines += n_top;
		...
		n_left = ( ( (int32_t)m_n_horiz_disstart - n_overscanleft ) * (int32_t)m_n_screenwidth ) / 2560;
		n_columns = ( ( ( (int32_t)m_n_horiz_disend - m_n_horiz_disstart ) * (int32_t)m_n_screenwidth ) / 2560 );
		...
		if( ( m_n_gpustatus & ( 1 << 0x15 ) ) != 0 )
		{
			...
			while( n_line > 0 )
			{
				uint16_t *p_n_src = m_p_p_vram[ n_y + m_n_displaystarty ] + n_x + n_displaystartx;
				uint16_t *p_n_dest = BITMAP_ADDR16(bitmap, n_y + n_top, n_x + n_left);
		...
		else
		{
			...
			while( n_line > 0 )
			{
            draw_scanline16( bitmap, n_x + n_left, n_y + n_top, n_columns, m_p_p_vram[ ( n_y + m_n_displaystarty ) & 1023 ] + n_x + m_n_displaystartx, NULL );
				n_y++;
				n_line--;
			}
		}
...
INLINE void decode_tpage( running_machine *machine, struct PSXGPU *p_psxgpu, uint32_t tpage )
{
	if( m_n_gputype == 2 )
	{
...
void psx_gpu_write( running_machine *machine, uint32_t *p_ram, int32_t n_size )
{
	while( n_size > 0 )
	{
		uint32_t data = *( p_ram );
...
void psx_gpu_read( running_machine *machine, uint32_t *p_ram, int32_t n_size )
{
	while( n_size > 0 )
	{
