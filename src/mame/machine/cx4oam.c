...
static void CX4_op00_00(running_machine *machine)
{
	...
	for(i = cx4.ram[0x620]; i > 0 && sprcount > 0; i--, srcptr += 16)
	{
		...
		if(memory_read_byte(cputag_get_address_space(machine, "maincpu", ADDRESS_SPACE_PROGRAM), spraddr))
		{
			...
			{
				x = (int8_t)memory_read_byte(cputag_get_address_space(machine, "maincpu", ADDRESS_SPACE_PROGRAM), spraddr + 1);
				if(sprattr & 0x40)
				{
				...
				if(x >= -16 && x <= 272)
				{
					y = (int8_t)memory_read_byte(cputag_get_address_space(machine, "maincpu", ADDRESS_SPACE_PROGRAM), spraddr + 2);
					if(sprattr & 0x80)
					{
