...
static fd1094_possibility *try_all_possibilities(const address_space *space, int basepc, int offset, int length, uint8_t *instrbuffer, uint8_t *keybuffer, fd1094_possibility *possdata);
...
static int validate_ea(const address_space *space, uint32_t pc, uint8_t modereg, const uint8_t *parambase, uint32_t flags);
static int validate_opcode(const address_space *space, uint32_t pc, const uint8_t *opdata, int maxwords);
...
INLINE uint8_t mask_for_keyaddr(offs_t address)
{
...
INLINE uint32_t advance_seed(uint32_t seed, int count)
{
...
INLINE uint8_t key_value_from_seed(uint32_t seed, uint8_t mask)
{
...
INLINE void generate_key_bytes(uint8_t *dest, uint32_t keyoffs, uint32_t count, uint32_t seed)
{
	int bytenum;
...
INLINE uint8_t get_opcode_length(uint16_t opcode)
{
...
INLINE void set_constraint(fd1094_constraint *constraint, uint32_t pc, uint16_t state, uint16_t value, uint16_t mask)
{
	constraint->pc = pc;
...
INLINE int pc_is_valid(const address_space *space, uint32_t pc, uint32_t flags)
{
...
INLINE int addr_is_valid(const address_space *space, uint32_t addr, uint32_t flags)
{
...
void fd1094_init_debugging(running_machine *machine, const char *cpureg, const char *keyreg, const char *statreg, void (*changed)(running_machine *))
{
	...
	/* set up the regions */
	coderegion = (uint16_t *)memory_region(machine, cpureg);
	coderegion_words = memory_region_length(machine, cpureg) / 2;
	keyregion = (uint8_t *)memory_region(machine, keyreg);
	keystatus = (uint16_t *)memory_region(machine, statreg);
	...
	ignorepc = auto_alloc_array_clear(machine, uint8_t, 1 << 23);

	/* allocate memory for the undo buffer */
	undobuff = auto_alloc_array(machine, uint8_t, keystatus_words * 2);
	memcpy(undobuff, keystatus, keystatus_words * 2);

	/* allocate memory for the possible seeds array */
	possible_seed = auto_alloc_array(machine, uint32_t, 65536);
