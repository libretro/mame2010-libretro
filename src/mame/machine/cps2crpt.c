...
static void cps2_decrypt(running_machine *machine, const uint32_t *master_key, uint32_t upper_limit)
{
	const address_space *space = cputag_get_address_space(machine, "maincpu", ADDRESS_SPACE_PROGRAM);
	uint16_t *rom = (uint16_t *)memory_region(machine, "maincpu");
	int length = memory_region_length(machine, "maincpu");
	uint16_t *dec = auto_alloc_array(machine, uint16_t, length/2);
	int i;
	uint32_t key1[4];
